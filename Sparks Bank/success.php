...
echo $amt;*/
  ...
  if($s_acc>=101 && $s_acc<=110)
  {
	if($r_acc>=101 && $r_acc<=110)
	{
	  ...
	  //select query 
	  $query1 = "SELECT * FROM users WHERE acc_no = '$s_acc'";
	...
	if($result1)
   {
	  if ($result1-> num_rows > 0) 
	  {
		...
		while($row = $result1->fetch_assoc()) 
		{
			...
			$edit = "update users set balance=balance - '$amt' where acc_no='$s_acc'";
	  ...
	  $query2 = "SELECT * FROM users WHERE acc_no = '$r_acc'";
	  ...
	  if ($result2-> num_rows > 0) 
	  {
		...
		{
			$edit2 = "update users set balance=balance + '$amt' where acc_no='$r_acc'";
